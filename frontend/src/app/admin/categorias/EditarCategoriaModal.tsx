...
export default function EditarCategoriaModal({ categoria, onClose, onSave }) {
  const { token } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/api/categorias/${categoria.id_categoria}`, {
        method: "PUT",
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md">
        <h3 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">Editar categoría</h3>
