  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        ...
        console.log('Cámaras filtradas:', camaras);
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white px-2 sm:px-4 md:px-8 py-8">
      <div className="max-w-screen-xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold mb-2 text-center sm:text-left">Cámaras</h1>
        <p className="mb-8 text-gray-300 text-center sm:text-left">Encuentra la cámara perfecta para cada momento.</p>
        ...
        {loading ? (
          <div className="text-center text-gray-400">Cargando cámaras...</div>
        ) : productos.length === 0 ? (
          <div className="text-center text-gray-400">
            {error ? "Error al cargar cámaras" : "No hay cámaras disponibles"}
          </div>
        ) : (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fadeIn px-2">
            <div className="bg-gray-900 rounded-xl shadow-2xl p-4 sm:p-8 max-w-xs sm:max-w-lg w-full relative scale-90 animate-growIn">
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="text-sm mb-1 text-gray-700"><span className="font-semibold">Categoría:</span> {productoSeleccionado.categoria?.nombre || "-"}</div>
                <div className="text-sm mb-1"><span className="font-semibold">Precio:</span> ${productoSeleccionado.precio}</div>
                <div className="text-sm mb-1 text-gray-700"><span className="font-semibold">Cantidad:</span> {productoSeleccionado.cantidad}</div>
                <button
